local extraIDs = {}
local extraModelIDs = {}
local modelElements = {} -- table of elements
...
function loadModelID(id, data)
	...
	if data and not data.model_id then
		...
		if data.model_id and data.model_txd and data.model_type=="object" then
			data.model_txd_id = engineRequestTXD(tostring(data.model_id))
			--local newModel = engineRequestModel("object", 1337)
			--local newTXD = engineRequestTXD("100200")
			--engineSetModelTXDID(newModel, newTXD)
		end
		if data.model_id then
			...
			addEventHandler("onClientResourceStop", resourceRoot, function()
				if extraModelIDs[data.model_id] == data.model then
					...
					extraModelIDs[data.model_id] = nil
					if data.model_txd_id then
						engineFreeTXD(data.model_txd_id)
						data.model_txd_id = nil
					end
				end
			end, false)
			if data.model_type=="vehicle" and data.model_info and (data.model_info.wheelSizeF or data.model_info.wheelSizeR) then
...
if false then -- vehicle upgrades
	local shader = dxCreateShader([[
		float4 partColor = float4(0, 0, 0, 0);
	...
	local part = createObject(1023, 0, 0, 0)
	...
	addEventHandler("onClientPreRender", root, function(tick)
		...
		--local comp = "boot_dummy"
		local comp = "bonnet_dummy"
		local comp = "ug_spoiler"
		local x, y, z = getVehicleComponentPosition(vehicle, comp, "root" or "world")
		--y = y+z_
		--y = y+(y_-y)/2
		local rx, ry, rz = getVehicleComponentRotation(vehicle, comp, "root" or "world")
		local a = getElementAlpha(vehicle)
		local r, g, b = getVehicleColor(vehicle, true)
		local l = getElementLighting(vehicle)*0.35+0.06
		dxSetShaderValue(shader, "partColor", r/255*l, g/255*l, b/255*l, a/255)
		--setElementPosition(part, x, y, z)
		--setElementRotation(part, rx, ry, rz)
		_ = x and attachElements(part, vehicle, x, y, z, rx, ry, rz)
		do return end
		print("FS:", getVehicleComponentPosition(vehicle, "boot_dummy", "parent"))
		--iprint("NAMES:", engineGetModelTextureNames(1023))
		local data = {} or getVehicleComponents(vehicle)
		for i, info in pairs(data) do
			local x, y, z = getVehicleComponentPosition(vehicle, i, "parent")
			local rx, ry, rz = getVehicleComponentRotation(vehicle, i, "parent")
			data[i] = {
				x=x, y=y, z=z,
			}
		end
		--setClipboard(inspect(data))
	end, true, "low")
end
...
function addElementRemover(res, element)
	if res then
		setElementParent(element, res)
		local f = function() destroyElement(element) end
		addEventHandler("onClientResourceStop", res, f, false)
		addEventHandler("onClientElementDestroy", element, function() removeEventHandler("onClientResourceStop", res, f) end, false)
	end
end
...
function cloneElement(element)
	if modelElements[element] then
		...
		if clone then
			addElementRemover(sourceResourceRoot, clone)
		...
		local element = _cloneElement(element)
		addElementRemover(sourceResourceRoot, element)
		return element
	end
end
...
function createObject(model, ...)
	...
	if data then
		...
		addElementRemover(sourceResourceRoot, object)
...
function createPed(model, ...)
	...
	if data then
		...
		addElementRemover(sourceResourceRoot, ped)
