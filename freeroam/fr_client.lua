...
do -- Extra Skins
	local start, t, loadCustomSkins = true
	function loadCustomSkins()
		local listControl = getControlData(wndSkin, 'skinlist')
		if not listControl.element then return end
		killTimer(t)
		start = false
		local listdata = g_gridListContents[listControl]
		local data = listdata.data
		local old = data[1]
		local new = {
			"group",
			depth = 1,
		...
		table.insert(data, 1, new)
		
		local list = new.children
		refreshList = function(model, type)
			--if type~="ped" then return end
			for i=#list, 1, -1 do list[i] = nil end
			local models = getExtraModels()
			for model, type in pairs(models) do
				if type=="ped" then
					local name = getVehicleNameFromModel(model) or engineGetModelNameFromID(model)
					list[#list+1] = {
						"ped",
						depth = 2,
						id = model,
						...
						shadow = {},
						siblings = list,
					} setmetatable(list[#list], getmetatable(old))
				end
			end
			_updateBindedGridList(listControl.element, listdata)
		end
		addEventHandler("onClientModelAdd", root, function()
			refreshList()
		end)
		addEventHandler("onClientModelRemove", root, function()
			refreshList()
		end)
		refreshList()
	end
	t = setTimer(loadCustomSkins, 1200, 0)
end
...
addCommandHandler('ss', setSkinCommand)function createSelectedVehicle(leaf)
	if not leaf then
...
do -- Extra Vehicles
	...
	function loadCustomVehicles()
		local listControl = getControlData(wndCreateVehicle, 'vehicles')
		if not listControl.element then return end
		killTimer(t)
		start = false
		local listdata = g_gridListContents[listControl]
		...
		local list = new.children
		refreshList = function(model, type)
			--if type~="vehicle" then return end
			for i=#list, 1, -1 do list[i] = nil end
			local models = getExtraModels()
			for model, type in pairs(models) do
	...
	t = setTimer(loadCustomVehicles, 1200, 0)
end
